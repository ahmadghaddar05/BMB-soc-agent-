...
// The model actually used for triage, given the chosen provider. The Hermes
// provider uses HERMES_MODEL; the Nous provider uses NOUS_MODEL (e.g. hy3:free).
function effectiveModel(config = runtimeConfig()) {
  return config.llmProvider === 'nous' ? config.nousModel : config.hermesModel;
}
...
function runtimeConfig(env = process.env) {
  return {
    ...
    // The application AI boundary is chosen per-deployment via LLM_PROVIDER.
    // 'hermes' is the default (strict run-orchestration API). 'nous' routes to
    // the Nous OpenAI-compatible chat endpoint (e.g. hy3:free) as a direct
    // fallback so a model can be swapped without standing up a Hermes server.
    llmProvider: env.LLM_PROVIDER || 'hermes',
    nousBaseUrl: (env.NOUS_BASE_URL || 'https://api.nousresearch.com/v1').replace(/\/+$/, ''),
    nousApiKey: env.NOUS_API_KEY || '',
    nousModel: env.NOUS_MODEL || 'hy3:free',
    nousJsonMode: bool(env.NOUS_JSON_MODE, true),
    nousRequestTimeoutMs: boundedInt(env.NOUS_REQUEST_TIMEOUT_MS, 120000, 5000, 600000),
...
function validateStartupConfig(config = runtimeConfig()) {
  ...
  if (!['hermes', 'nous'].includes(config.llmProvider)) errors.push('LLM_PROVIDER must be hermes or nous');
  if (config.llmProvider === 'nous' && !config.nousApiKey) errors.push('NOUS_API_KEY is required when LLM_PROVIDER=nous');
  if (!/^[A-Za-z0-9._:/-]{1,200}$/.test(config.nousModel)) errors.push('NOUS_MODEL contains unsupported characters');
  return { ok: errors.length === 0, errors, warnings };
}

module.exports = { bool, boundedInt, runtimeConfig, validateStartupConfig, effectiveModel, configFromSettings };

// Overlays DB-backed settings (from the dashboard) on top of the env-derived
// config so a model/provider change takes effect on the next cycle without a
// process restart. Only the dashboard-editable AI keys are overridden.
function configFromSettings(settings = {}, base = runtimeConfig()) {
  const config = { ...base };
  if (typeof settings.llm_provider === 'string' && ['hermes', 'nous'].includes(settings.llm_provider)) {
    config.llmProvider = settings.llm_provider;
  }
  if (typeof settings.nous_model === 'string' && /^[A-Za-z0-9._:/-]{1,200}$/.test(settings.nous_model)) {
    config.nousModel = settings.nous_model;
  }
  return config;
}
