...
function defaultHermesClient() {
  ...
  const key = JSON.stringify({
    ...
    nousUrl: config.nousBaseUrl, nousModel: config.nousModel,
  ...
  if (!singleton || key !== singletonKey) {
    singleton = config.llmProvider === 'nous'
      ? createNousClient({ config })
      : createHermesClient({ config });
    singletonKey = key;
  }
  return singleton;
}

// Nous-backed client: same surface as the Hermes client so triage.js / chat.js
// are provider-agnostic. Tool-calling modes are intentionally unsupported here;
// the screening policy in triage.js rejects unexpected tool requests.
function createNousClient({ config = runtimeConfig(), fetchImpl = global.fetch } = {}) {
  const { runNousCompletion, handshakeNous } = require('./nous');
  return {
    baseUrl: config.nousBaseUrl,
    handshake: () => handshakeNous({ config }),
    request: async () => { throw new HermesError('HERMES_UNSUPPORTED', 'Direct requests are not used by the Nous provider', { status: 503 }); },
    runAgent: (options) => runNousCompletion({ ...options, config, fetchImpl }),
    stopRun: async () => true,
  };
}

module.exports = { createHermesClient, defaultHermesClient, createNousClient, isForbiddenTool, normalizedBaseUrl };
