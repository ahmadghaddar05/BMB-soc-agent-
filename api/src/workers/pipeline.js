...
const { runtimeConfig, effectiveModel, configFromSettings } = require('../config');
const {
...
async function triagePending(settings, limit = 50, alertId = null, {
  ...
  for (const row of rows) {
    ...
    const activeModel = effectiveModel(config);
    const { cacheKey, enrichmentHash } = triageCacheIdentity(row, sig, activeModel);
    try {
      ...
      if (useCache && !bypassCache) {
        const c = await db.query(
          ...
          [cacheKey, sig, enrichmentHash, PROMPT_VERSION,
            OUTPUT_SCHEMA_VERSION, activeModel]
        );
        if (c.rows.length) {
        ...
        if (useCache) {
          await db.query(
            ...
            [cacheKey, row.rule_id, verdict, sig, PROMPT_VERSION,
              OUTPUT_SCHEMA_VERSION, activeModel, enrichmentHash,
