...
export default function AgentPerformanceHub({ agent = {}, collector = {}, loading = false, error = null, onReview }) {
  const current = error
    ...
    : loading ? { stage:'unknown', message:'Checking AI-assisted workflow status', active:false }
  ...
  return (
    <section className="rounded-2xl border border-[#1d374b] bg-[#0b1722] p-5 shadow-[0_18px_45px_rgba(0,0,0,.14)] md:p-6" aria-labelledby="agent-performance-title">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#1a3346] pb-5"><div><p className="text-xs font-semibold uppercase tracking-[.12em] text-[#9b8afb]">AI-assisted workflow status</p><h2 id="agent-performance-title" className="mt-1.5 text-lg font-semibold tracking-[-.02em] text-[#f0f6fa]">AI Workflow Hub</h2><p className="mt-1 text-sm text-[#8098aa]">Internal triage, correlation, and investigation support. External response remains human-approved and simulation-only.</p></div><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold ${error || loading || agent.enabled == null ? 'border-[#7891a5]/30 bg-[#7891a5]/[.07] text-[#a8bac7]' : agent.enabled ? 'border-[#33d69f]/30 bg-[#33d69f]/[.07] text-[#50dcae]' : 'border-[#f7b955]/30 bg-[#f7b955]/[.07] text-[#f7c66d]'}`}><i className={`h-1.5 w-1.5 rounded-full ${error || loading || agent.enabled == null ? 'bg-[#7891a5]' : agent.enabled ? 'bg-[#33d69f] shadow-[0_0_8px_#33d69f]' : 'bg-[#f7b955]'}`} />{error ? 'Status unavailable' : loading || agent.enabled == null ? 'Checking status' : agent.enabled ? 'Workflows enabled' : 'Workflows disabled'}</span></div>
