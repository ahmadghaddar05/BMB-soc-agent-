import {
  AlertOctagon, ArrowRight, Bot, Building2, Clock3, Gauge, HelpCircle,
} from 'lucide-react';
...
function MetricCard({ icon:Icon, title, value, suffix, available = true, reason, definition, confidence, target, previous, lowerIsBetter, onOpen, tone = 'blue', estimated = false }) {
  const tones = {
    blue:'bg-[#3988ff]/10 text-[#65a4ff]',
    red:'bg-[#ef4453]/10 text-[#ff7480]',
    amber:'bg-[#f2c94c]/10 text-[#f4d267]',
    green:'bg-[#25cf91]/10 text-[#43d5a2]',
    purple:'bg-[#8a6cff]/10 text-[#aa97ff]',
  };
  return (
    <article className="flex min-h-[202px] flex-col rounded-2xl border border-[#17334a] bg-[#081725] p-4 shadow-[0_14px_36px_rgba(0,0,0,.16)]">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[#7290a8]" title={definition}><HelpCircle size={12} />{estimated ? 'Estimate' : 'Definition'}</span>
      </div>
      ...
      <div className="mt-3 min-h-10 text-xs leading-5 text-[#7891a5]">
        {available
          ? estimated
            ? <span>Modelled from completed AI-assisted workflow records</span>
            : <Delta current={value} previous={previous} lowerIsBetter={lowerIsBetter} />
          : <span>{reason || 'Required source data is unavailable.'}</span>}
      </div>
...
export default function ExecutiveKpiGrid({ overview, onOpenRisks, onOpenAssets, onOpenMethodology, onOpenAutomation }) {
  ...
  const workload = metrics.estimated_analyst_time_saved || metrics.analyst_workload_reduced || { value:overview?.time_saved?.hours, previous_period:overview?.time_saved?.previous_period_hours, available:true, confidence:'estimated' };
  const exposureValue = Number(exposure.value);
  const exposureTone = exposureValue >= 75 ? 'red' : exposureValue >= 40 ? 'amber' : 'blue';
