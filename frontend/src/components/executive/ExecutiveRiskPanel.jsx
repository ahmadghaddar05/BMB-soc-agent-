  ...
  return (
    <section className="rounded-2xl border border-[#17334a] bg-[#081725] p-5" aria-labelledby="top-risk-title">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-[#7891a5]">Leadership attention</p>
          <h2 id="top-risk-title" className="mt-1 text-lg font-semibold text-[#edf5fa]">Priority security risks</h2>
          <p className="mt-1 text-xs leading-5 text-[#668299]">Prioritized from stored incident severity. Business impact remains a proxy until service criticality is mapped.</p>
        </div>
        ...
        {items.length ? items.slice(0, 3).map(item => (
          <button key={item.id} type="button" onClick={event => onSelect?.(item, event.currentTarget)} className="group grid w-full gap-3 rounded-xl border border-[#173248] bg-[#071521] p-3.5 text-left transition hover:border-[#315a78] sm:grid-cols-[minmax(0,1fr)_150px_auto] sm:items-center">
            <span className="min-w-0">
              <span className="flex items-center gap-2"><BriefcaseBusiness size={14} className="shrink-0 text-[#65a4ff]" /><strong className="truncate text-sm font-semibold text-[#dce8f0]">{item.title || `Incident ${item.id}`}</strong></span>
              <span className="mt-1.5 block text-xs text-[#708ca2]">{item.required_decision || 'Confirm ownership and next action'}</span>
            </span>
            <span className="flex items-center gap-1.5 text-xs text-[#8ba3b6]"><UserRound size={13} />{item.owner || 'Owner required'}</span>
            <span className="flex items-center gap-2">
              <i className={`rounded-full border px-2 py-1 text-[10px] font-semibold not-italic capitalize ${priorityClass(item.severity)}`}>{humanize(item.severity || 'unknown')} priority</i>
              <ArrowRight size={14} className="text-[#56758c] transition group-hover:translate-x-0.5" />
            </span>
          </button>
