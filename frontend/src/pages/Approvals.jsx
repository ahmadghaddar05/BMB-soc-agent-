...
function statusLabel(item = {}) {
  if (item.status === 'executed') {
    if (item.action_type === 'response.rollback') return 'Simulation reverted';
    if (item.action_type === 'response.simulate') return 'Simulation activated';
    return 'Applied internally';
  }
  if (item.status === 'pending') return 'Pending review';
...
export default function Approvals() {
  ...
  async function load(nextStatus = status) {
    ...
    try {
      ...
      setError(loadError.message || 'Approval requests could not be loaded.');
  ...
  async function decide(decision) {
    ...
    try {
      ...
      setError(decisionError.message || 'The approval decision could not be recorded.');
  ...
  return <div className="module-page approvals-page">
    ...
    <div className="approval-layout">
      ...
      <section className="module-panel approval-detail">
        ...
        {selected ? <>
          <div className="approval-detail-head"><div><small>{actionReference(selected)}</small><h3>{LABELS[selected.action_type] || selected.action_type}</h3><p>{friendlyEvidenceText(selected.reason)}</p></div><span className={`approval-chip ${statusTone(selected.status)}`}>{statusLabel(selected)}</span></div>
        ...
        </> : error ? <div className="module-empty"><ShieldCheck /><strong>Approval data unavailable</strong><span>Retry loading before making a decision.</span></div> : <div className="module-empty"><ShieldCheck /><strong>Select a request</strong><span>Review the intended effect, evidence, target, and exact safety boundary before deciding.</span></div>}
      </section>
    </div>
  </div>;
}
