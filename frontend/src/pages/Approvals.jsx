...
export default function Approvals() {
  ...
  return <div className="module-page approvals-page">
    ...
    <div className="approval-policy"><ShieldCheck /><div><strong>Phase 9 safety boundary</strong><span>Workflow changes and every simulated response require approval. Simulations update only the BMB ledger; external isolation, account disablement, and IP blocking remain unavailable.</span></div></div>
    ...
    <div className="approval-layout">
      ...
      <section className="module-panel approval-detail">
        {selected?.preview && <div className="approval-preview"><strong>Execution preview</strong><p>{selected.preview.intended_effect}</p><dl><div><dt>Mode</dt><dd>{selected.preview.mode}</dd></div><div><dt>Connector</dt><dd>{selected.preview.connector}</dd></div><div><dt>External effects</dt><dd>{selected.preview.external_side_effects ? 'Possible' : 'None'}</dd></div><div><dt>Reversible</dt><dd>{selected.preview.reversible ? 'Yes' : 'No'}</dd></div></dl></div>}
