...
export default function Integrations() {
  ...
  const cards = useMemo(() => [
    ...
    { id:'Hermes', icon:Bot, name:'Hermes AI analyst', category:'Decision intelligence', status:healthStatus(services.hermes), detail:'Hermes Runs API capability, model, and safe host-tool profile check.', metrics:[['Status',services.hermes?.status],['Safe profile',services.hermes?.safe === true ? 'Yes' : 'No'],['Model',services.hermes?.model || '—'],['Latency',services.hermes?.latency_ms != null ? `${services.hermes.latency_ms} ms` : '—']], path:'/health/dependencies' },
