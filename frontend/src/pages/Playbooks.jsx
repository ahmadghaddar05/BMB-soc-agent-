...
export default function Playbooks() {
  ...
  return <div className="module-page playbooks-page">
    <div className="module-hero compact"><div><span className="eyebrow"><BookOpenCheck />Response orchestration</span><h2>Playbooks</h2><p>Guided, trackable analyst procedures for common security incidents.</p></div><span className="live-pill"><i />{activeRuns} playbook{activeRuns === 1 ? '' : 's'} in progress</span></div>
    <div className="playbook-layout"><section className="module-panel playbook-catalog"><div className="panel-heading"><div><BookOpenCheck /><span><strong>Response catalog</strong><small>{PLAYBOOKS.length} approved workflows</small></span></div></div><div className="playbook-list">{PLAYBOOKS.map(item => { const Icon=item.icon; const itemRun=runs[item.id]; return <button key={item.id} className={activeId === item.id ? 'active' : ''} onClick={() => setActiveId(item.id)}><span className={`playbook-icon severity-${item.severity}`}><Icon /></span><div><strong>{item.name}</strong><small>{item.category} · {item.time}</small></div>{itemRun && <em className={itemRun.status}>{itemRun.status}</em>}<ChevronRight /></button>; })}</div></section>
      <section className="module-panel playbook-run"><div className="playbook-head"><span className={`playbook-icon large severity-${active.severity}`}>{<active.icon />}</span><div><small>{active.category} response</small><h3>{active.name}</h3><p>{active.trigger}</p></div><span className="playbook-duration"><Clock3 />{active.time}</span></div><div className="playbook-progress"><span><b style={{width:`${run ? progress : 0}%`}} /></span><small>{run ? `${completed.length} of ${active.steps.length} steps complete` : 'Not started'}</small></div><div className="playbook-steps">{active.steps.map((step,index) => <button key={step} className={completed.includes(index) ? 'complete' : ''} onClick={() => toggleStep(index)} disabled={!run}><span>{completed.includes(index) ? <Check /> : index + 1}</span><div><strong>{step}</strong><small>{completed.includes(index) ? 'Completed by Analyst' : 'Awaiting analyst action'}</small></div></button>)}</div><div className="playbook-footer">{!run ? <button className="primary-action" onClick={start}><Play />Start playbook</button> : <><span className={`run-status ${run.status}`}>{run.status === 'complete' ? 'Playbook completed' : 'Playbook in progress'}</span><button className="ghost-action" onClick={reset}><RotateCcw />Reset run</button></>}</div></section></div>
  </div>;
}
