...
export default function Settings() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      {/* ── LLM ── */}
      <Section title="AI triage provider">
        <Row label="Provider" hint="Hermes is the strict run-orchestration engine. Nous is a direct OpenAI-compatible model (e.g. hy3:free) used as a fallback so a model can be swapped without standing up a Hermes server.">
          <select className="select w-56" {...field('llm_provider')}>
            <option value="hermes">Hermes Agent</option>
            <option value="nous">Nous (hy3:free / OpenAI-compatible)</option>
          </select>
        </Row>
        <Row label="Nous model" hint="Only used when Provider = Nous. Free tier is hy3:free.">
          <input className="input w-56" type="text" placeholder="hy3:free" {...field('nous_model')} />
        </Row>
        ...
        <button className="btn-primary" disabled={loading}
          onClick={()=>save({
            llm_provider:s.llm_provider || 'hermes',
            nous_model:s.nous_model || 'hy3:free',
          ...
          <Save className="w-4 h-4"/> Save AI triage policy
        </button>
      </Section>
