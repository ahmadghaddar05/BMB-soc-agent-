...
import ConnectorManager from '../components/admin/ConnectorManager';
    ...
    {loading && !runtime && !settings ? <section className="module-panel"><div className="module-empty small" role="status"><RefreshCw className="animate-spin" /><strong>Loading platform configuration</strong></div></section> : <>
      ...
      <section className="module-panel" style={{ marginTop:10 }}>
        ...
        <div className="module-table-wrap"><table className="module-table"><thead><tr><th>Area</th><th>Current state</th><th>Managed through</th><th>Open workspace</th></tr></thead><tbody>
          <tr><td><strong>Security telemetry</strong><small>Elastic, Splunk, Wazuh, or lab source</small></td><td><StatusBadge tone="neutral">{runtime?.alert_source?.type || 'Not reported'}</StatusBadge></td><td>{runtime?.alert_source?.configuration_source === 'managed_connector' ? 'Encrypted dashboard connector' : 'Environment fallback'}</td><td><button type="button" className="table-link" onClick={() => navigate('/collector-health')}>Collector Health <ExternalLink /></button></td></tr>
      ...
      <div className="module-notice" role="status"><Bot /><span>Connector credentials are encrypted server-side and never returned to the browser. Source tests, activation, updates, and deletion are written to the governance audit trail.</span></div>
    </>}
  </div>;
}
